...
struct cluster freecluschain(struct directory entry){
	struct cluster info;
	unsigned int fatOffset = 0;
	unsigned long next_cluster = 0;
	unsigned long byte_addr;
	...
	for(int i=0; i < entry.cluster->clusterNum; i++){
		while(searching){
			fatOffset = cluster * 4;
			...
			printf("Byte Addr for free Cluster 0x%x", byte_addr);
